...
const CC isqrt3 = 1.il*std::sqrt<long double>(3.l);
                    ...
                    CC view_corner_tr)
{
  for(unsigned short px_x = start_px_x; px_x < end_px_x; px_x++)
  {
    for(unsigned short px_y = 0; px_y < px_height; px_y++)
    {
      ...
      auto pixel_lengths = lengths_from_word_tree(word_tree_from_matrices(representation((-2.l - 2.il) + (3.l + 2.il)*t + isqrt3*t,
                                                                                         (1.l - 1.il) - (3.l - 1.il)*t + 2.l*isqrt3*t,
                                                                                         1)));
...
int main()
{
  auto tree = word_tree_from_matrices(representation(5, 3il, 1));
  ...
  for(unsigned short lr = 0; lr < 20; lr++)
    threads.push_back(std::thread([=, &base_point_lengths]() {vertical_strip(base_point_lengths, px_width, px_height, lr*20, lr*20 + 20, -1.L-2.il, 3.L+2.il);} ));
