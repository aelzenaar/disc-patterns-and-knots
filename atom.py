...
class AtomGroup(cayley.GroupCache):
    def __init__(self, n, M, speedfactor = 0.8):
        ...
        def walk_a_spiral(this_r, other_r, start_θ):
            ...
            return [(complex(centre), float(radius)) for _,centre,radius in circles]
...
if __name__ == '__main__':
    ...
    with multiprocessing.Pool(5, maxtasksperchild=1) as pool:
    ...
    w, r = G.circles2[10]
    def do_random(x):
        z = complex(r**2/(x['x'] - 1j*x['y'] + w.conjugate()) + w)
        x['x'] = z.real
        x['y'] = z.imag
        return x

    df = df.apply(do_random, axis=1, meta={'x': 'float16', 'y': 'float16', 'colour': 'float16'})

    print(f"    atom.py has done reflection again")
    scatter = hv.Scatter(df, kdims = ['x'], vdims = ['y','colour']).opts(marker='dot', frame_width=width, frame_height=width, size=.1, color='black')\
        .redim(x=hv.Dimension('x', range=(0.8,1.6)),y=hv.Dimension('y', range=(-.4, .4)))
        # .redim(x=hv.Dimension('x', range=(-2.1,2.1)),y=hv.Dimension('y', range=(-2.1, 2.1)))

    print("    atom.py is starting to shade plot")
    ResampleOperation2D.width=width
    ResampleOperation2D.height=width
    shaded = datashade(scatter, min_alpha=1, cmap = ['black']).opts(width=width, height=width, bgcolor="white",xaxis=None,yaxis=None)
    ...
    if add_bead_annotations:
        ...
        shaded *= pairsToCircles(G.circles1).opts(color='#be1e2d')
        shaded *= pairsToCircles(G.circles2).opts(color='#21409a')
