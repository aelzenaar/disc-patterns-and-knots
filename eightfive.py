...
α = 0.5*(3-1j*mp.sqrt(7))
β = 0.5*(7-1j*mp.sqrt(7))
λ = 0.25*(3-1j*mp.sqrt(7))
...
circles0 = [G.subgroup([(6,5,2,2)]).isometric_circle(w) for w in G.subgroup([(6,5,2,2)]).free_cayley_graph_bfs(1)]
circles0 += [G.subgroup([(2,5)]).isometric_circle(w) for w in G.subgroup([(2,5)]).free_cayley_graph_bfs(1)]

# circles0 += [G.subgroup([(5,2)]).isometric_circle(w) for w in G.subgroup([(5,2)]).free_cayley_graph_bfs(1)]
# circles0 += [G.subgroup([(2,2,6,5)]).isometric_circle(w) for w in G.subgroup([(2,2,6,5)]).free_cayley_graph_bfs(1)]

circles0 += [G.subgroup([(7,0,1,0,7,0,1)]).isometric_circle(w) for w in G.subgroup([(7,0,1,0,7,0,1)]).free_cayley_graph_bfs(1)]
circles0 += [G.subgroup([(7,0,1,1,7,0,1)]).isometric_circle(w) for w in G.subgroup([(7,0,1,1,7,0,1)]).free_cayley_graph_bfs(1)]


# circles0 += [G.subgroup([(7,0,1)]).isometric_circle(w) for w in G.subgroup([(7,0,1)]).free_cayley_graph_bfs(1)]

# circles0 += [G.subgroup([(7,0,1,0,1,7,0,1,3,4)]).isometric_circle(w) for w in G.subgroup([(7,0,1)]).free_cayley_graph_bfs(1)]
# circles0 += [G.subgroup([(7,0,1,2,5,7,0,1)]).isometric_circle(w) for w in G.subgroup([(7,0,1,2,5,7,0,1)]).free_cayley_graph_bfs(1)]
# circles0 += [G.subgroup([(7,0,1,6,5,2,2,7,0,1)]).isometric_circle(w) for w in G.subgroup([(7,0,1,2,5,7,0,1)]).free_cayley_graph_bfs(1)]

circles = []
for i in [-1,0,1]:
    for j in [-1,0,1]:
        circles += [(cen + i*α + j*β, rad) for cen, rad in circles0]
...
scatter = datashade(hv.Scatter(limit_set_G, kdims = ['x'], vdims = ['y','colour']).opts(data_aspect=1, marker = "dot", size = .01)\
            .redim(x=hv.Dimension('x', range=(-2,2)),y=hv.Dimension('y', range=(-2, 2))), width=800, cmap=['silver'], min_alpha=1)\
...
for s in circle_scatters:
    scatter *= s
hv.save(scatter.opts(width=800,data_aspect=1), "eightfive_knot_limit_new.png")
